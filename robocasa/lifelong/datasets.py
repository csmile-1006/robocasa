...
def get_dataset(
    ...
    **kwargs,
    ...
    for demo_id in dataset.demos:
        ep_meta = json.loads(dataset.hdf5_file["data"][demo_id].attrs["ep_meta"])
        task_descriptions[demo_id] = ep_meta["lang"]

    return dataset, task_descriptions, shape_meta


class SequenceVLDataset(Dataset):
    def __init__(self, sequence_dataset, task_embs):
    ...
    def __getitem__(self, idx):
        demo_id = self.sequence_dataset._index_to_demo_id[idx]
        return_dict = self.sequence_dataset.__getitem__(idx)
        return_dict["task_emb"] = self.task_embs[demo_id]
        return return_dict
