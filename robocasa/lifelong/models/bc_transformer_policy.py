...
from robocasa.lifelong.models.modules.rgb_modules import *
from robocasa.lifelong.models.modules.language_modules import *
from robocasa.lifelong.models.modules.transformer_modules import *
from robocasa.lifelong.models.base_policy import BasePolicy
from robocasa.lifelong.models.policy_head import *
...
class ExtraModalityTokens(nn.Module):
    def __init__(
        ...
        base_dim = 3 + 4
        joint_states_dim = 7 + 7 + 7 + 7
        gripper_states_dim = 2 + 2
        ee_dim = 3 + 3 + 4 + 4

        self.num_extra = int(use_joint) + int(use_gripper) + int(use_ee) + int(use_base)

        extra_low_level_feature_dim = (
            int(use_base) * base_dim
        ...
        for (proprio_dim, use_modality, modality_name) in [
            (base_dim, self.use_base, "base_states"),
    ...
    def forward(self, obs_dict):
        ...
        for (use_modality, modality_name, modality_names) in [
            (self.use_base, "base_states", ["robot0_base_pos", "robot0_base_quat"]),
            (self.use_ee, "ee_states", ["robot0_eef_pos", "robot0_base_to_eef_pos", "robot0_eef_quat", "robot0_base_to_eef_quat"]),
            (self.use_joint, "joint_states", ["robot0_joint_pos", "robot0_joint_pos_sin", "robot0_joint_pos_cos", "robot0_joint_vel"]),
            (self.use_gripper, "gripper_states", ["robot0_gripper_qpos", "robot0_gripper_qvel"]),
        ]:

            if use_modality:
                _input = torch.cat([obs_dict[modality_name] for modality_name in modality_names], dim=-1)
                tensor_list.append(
