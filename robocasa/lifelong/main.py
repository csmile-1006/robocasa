...
from robocasa.lifelong.algos import get_algo_class, get_algo_list
from robocasa.lifelong.models import get_policy_list
from robocasa.lifelong.datasets import GroupedTaskDataset, SequenceVLDataset, get_dataset
from robocasa.lifelong.metric import evaluate_loss, evaluate_success
...
def main(hydra_cfg):
    ...
    for i in range(n_manip_tasks):
        ...
        try:
            task_i_dataset, task_descriptions, shape_meta = get_dataset(
                dataset_path=cfg.data.dataset_paths[i],
            ...
            print(
                f"[error] failed to load task {i} from {cfg.data.dataset_paths[i]}"
            )
            print(f"[error] {e}")
        # add language to the vision dataset, hence we call vl_dataset
        descriptions.append(task_descriptions)
    ...
    assert gsz == 1, "task_group_size must be 1 for now"
    if gsz == 1:  # each manipulation task is its own lifelong learning task
        datasets = [
            SequenceVLDataset(ds, embs) for (ds, embs) in zip(manip_datasets, task_embs)
        ]
    ...
    cfg.benchmark_name = "-".join([path.split("/")[-3] for path in cfg.data.dataset_paths])
    ...
    if cfg.lifelong.algo == "Multitask":
        ...
        algo.train()
        s_fwd, l_fwd = algo.learn_all_tasks(datasets, None, result_summary)
        ...
        for i in range(n_tasks):
            ...
            s_fwd, l_fwd = algo.learn_one_task(
                datasets[i], i, None, result_summary
            )
