...
from robocasa.lifelong.metric import *
from robocasa.lifelong.models import *
...
class Sequential(nn.Module, metaclass=AlgoMeta):
    ...
    def learn_one_task(self, dataset, task_id, benchmark, result_summary):
        ...
        # start training
        for epoch in trange(0, self.cfg.train.n_epochs + 1, desc=f"Task {task_id}", leave=False):
            t0 = time.time()
            ...
            if epoch % 10 == 0:
                torch_save_model(self.policy, model_checkpoint_name[:-4] + f"_epoch{epoch}" + ".pth", cfg=self.cfg)
