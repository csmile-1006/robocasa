...
def dataset_states_to_obs_single(args):
    ...
    # Process each demo sequentially
    f = h5py.File(args.dataset, "r")
    f_out = h5py.File(output_path, "w")

    for ind, ep in tqdm(enumerate(demos), total=len(demos), desc="Processing demos"):
        try:
            ep_grp = f["data/{}".format(ep)]
            ...
            write_traj_to_file_single(f_out, ep, traj)
            ...
            print("_" * 50)
            del env
            env = EnvUtils.create_env_for_data_processing(  # noqa
                env_meta=env_meta,
                camera_names=args.camera_names,
                camera_height=args.camera_height,
                camera_width=args.camera_width,
    ...
    f.close()
    f_out.close()
    print("Finished processing {} demonstrations".format(num_demos))
    return
