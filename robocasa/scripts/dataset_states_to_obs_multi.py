...
def write_traj_to_file_single(f_out, ep, traj):
    """Write a single trajectory to the output HDF5 file."""
    ep_data_grp = f_out.create_group("data/{}".format(ep))
...
def write_traj_to_file_multiprocessing(
    args, output_path, total_samples, total_run, processes, mul_queue
):
    """Write trajectories to file in a separate process for multiprocessing."""
    ...
    try:
        while (total_run.value < processes) or not mul_queue.empty():
            if not mul_queue.empty():
                ...
                print(
                    "ep {}: wrote {} transitions to group {} at process {} with {} finished. Processing rate: {:.2f} sec/demo".format(
                        num_processed,
    ...
    for num_demos in [
        10, 20, 30, 40, 50, 60, 70, 75, 80, 90, 100, 125, 150, 200, 250, 300, 400, 500,
        600, 700, 800, 900, 1000, 1500, 2000, 2500, 3000, 4000, 5000, 10000,
    ]:
...
def extract_multiple_trajectories_with_error(
    ...
    """Extract multiple trajectories with error handling for multiprocessing."""
...
def dataset_states_to_obs_single(args):
    """Main function that uses multiprocessing for faster trajectory processing."""
    ...
    # Add all demo indices to work queue
    ...
    # Create file writer process
    process1 = multiprocessing.Process(
        target=write_traj_to_file_multiprocessing,
        args=(
            args,
            output_path,
    ...
    print("Finished multiprocessing {} demonstrations".format(num_demos))
    return
...
if __name__ == "__main__":
    ...
    dataset_states_to_obs_single(args)